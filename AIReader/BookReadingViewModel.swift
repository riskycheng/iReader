...
class BookReadingViewModel: ObservableObject {
        ...
        private func loadChapterContent() async {
            do {
                ...
                if let preloadedContent = preloadedChapters[chapterIndex] {
                    ...
                    if FileManager.default.fileExists(atPath: chapterURL) || chapterURL.hasPrefix("file://") {
                        print("Loading local book content from: \(chapterURL)")
                        
                        // Handle local file loading with better error handling
                        do {
                            ...
                            print("Error loading local file: \(error.localizedDescription)")
                            // Create a simple placeholder content for local files that failed to load
                            let errorContent = "　　无法加载本地文件内容，请确认文件是否存在或有权限访问。\n\n　　错误信息：\(error.localizedDescription)"
                            
                            await MainActor.run {
                                self.currentChapterContent = errorContent
                                self.splitContentIntoPages(errorContent)
                                self.errorMessage = nil // Don't show error dialog, show in content instead
                                self.isLoading = false
                            }
                        }
                    } else {
                        // Handle remote URL content
                        let content = try await fetchChapterContent(from: chapterURL)
                ...
                print("Chapter loading error: \(error.localizedDescription)")
                await MainActor.run {
        ...
        private func fetchChapterContent(from urlString: String) async throws -> String {
            // First check if this is a direct file path (no URL scheme)
            if FileManager.default.fileExists(atPath: urlString) {
                print("Loading local file directly from path: \(urlString)")
                return try await loadLocalFile(path: urlString)
            }
            
            // Then check if this is a file:// URL
            if urlString.hasPrefix("file://") {
                print("Converting file URL to direct path: \(urlString)")
                ...
                if FileManager.default.fileExists(atPath: filePath) {
                    return try await loadLocalFile(path: filePath)
                } else {
        ...
        private func loadLocalFile(path: String) async throws -> String {
            ...
            do {
                ...
                // For large files (over 2MB), only load the first part to avoid performance issues
                if fileSize > 2 * 1024 * 1024 { // 2MB threshold
                    print("Large file detected (\(fileSize / 1024 / 1024)MB). Loading only first 200KB.")
                    
                    // Read only the first part of the file
                    let fileHandle = try FileHandle(forReadingFrom: URL(fileURLWithPath: path))
                    let sampleData = fileHandle.readData(ofLength: 200 * 1024) // 200KB sample
                    try fileHandle.close()
                    
                    // Try to decode the sample with different encodings
                    for encoding in [String.Encoding.utf8, .windowsCP1252, .isoLatin1, .ascii] {
                        if let content = String(data: sampleData, encoding: encoding) {
                            print("Successfully decoded large file sample with encoding: \(encoding)")
                            let notice = "\n\n【注意：这是一个大型文件 (\(fileSize / 1024 / 1024)MB)，为了提高性能，只显示前部分内容。】\n\n"
                    ...
                    throw NSError(domain: "FileError", code: 0, userInfo: [NSLocalizedDescriptionKey: "Unable to decode large file content with any supported encoding"])
                }
                
                // For normal sized files, load the entire content
                let data = try Data(contentsOf: URL(fileURLWithPath: path))
                print("Successfully read \(data.count) bytes from local file")
                
                // Try different encodings
                for encoding in [String.Encoding.utf8, .windowsCP1252, .isoLatin1, .ascii] {
                    if let content = String(data: data, encoding: encoding) {
                        print("Successfully decoded file with encoding: \(encoding)")
                ...
                throw NSError(domain: "FileError", code: 0, userInfo: [NSLocalizedDescriptionKey: "Unable to decode file content with any supported encoding"])
            } catch {
